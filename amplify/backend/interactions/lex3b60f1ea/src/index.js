...
exports.handler = function (event, context) {
  const lex = new aws.LexModelBuildingService({
    ...
    region: event.ResourceProperties.lexRegion
  });
  if (event.RequestType == 'Delete') {
    let botName = 'EECampBot';
    if (process.env.ENV && process.env.ENV !== 'NONE') {
      botName = botName + '_' + process.env.ENV;
    }
    lex
      .deleteBot({ name: botName })
      .promise()
      .then(() => {
  ...
  let intentParams = [
    {
      name: 'when' + '_' + process.env.ENV,

      sampleUtterances: ['book'],
  ...
  let botParams = {
    ...
    intents: [
      {
        intentName: 'when' + '_' + process.env.ENV,
  ...
  checkAndCreateLexServiceRole()
    .then(() => {
      return getSlotTypes(newSlotTypeParams, lex);
    })
    .then(() => {
      return putSlotTypes(newSlotTypeParams, lex);
    })
    .then(() => {
      return getIntents(intentParams, lex);
    })
    .then(() => {
      return putIntents(intentParams, lex);
    })
    .then(() => {
      return getBot(botParams, lex);
    })
    .then(() => {
      return putBot(botParams, lex);
    })
    .then((res) => {
...
function checkAndCreateLexServiceRole() {
  return checkIfLexServiceRoleExists().then((roleExists) => {
    if (!roleExists) {
...
function createNewLexServiceRole() {
  ...
  const params = {
    ...
    Description:
      'Allows Amazon Lex to create and manage voice enabled bots on your behalf'
  };
...
function ensureLambdaFunctionAccess(intent) {
  if (intent.fulfillmentLambda) {
    const { region, accountId, lambdaArn, lambdaName } =
      intent.fulfillmentLambda;

    const params = {
      FunctionName: lambdaName,
      StatementId: `Lex-${intent.name}` + '3a36be1a',
